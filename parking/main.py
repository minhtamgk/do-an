...
class Main(Ui_MainWindow):
    ...
    def toggle_pause(self):
        if self.paused:
            self.paused = False
            self.timer.start(3)  # Tiếp tục cập nhật video
            self.button_pause.setText("Tạm dừng")  # Thay đổi chữ của nút
        else:
            self.paused = True
            self.timer.stop()  # Dừng cập nhật video
            self.button_pause.setText("Tiếp tục")  # Thay đổi chữ của nút
